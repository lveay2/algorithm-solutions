...
public class _261_Maximum_Connected_Area {
    ...
    private static Map<Point, Point> p2p = new HashMap<>();
    private static Map<Point, Integer> p2Area = new HashMap<>();
    ...
    private static int bfs(int[][] matrix, int x, int y, int n, int m, boolean[][] visited) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (0 <= newX && newX < n && 0 <= newY && newY < m && !visited[newX][newY] && matrix[newX][newY] == 1) {
