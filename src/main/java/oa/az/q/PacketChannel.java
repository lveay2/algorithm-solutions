...
public class PacketChannel {
  ...
  private static long maximumQuality(List<Integer> packets, int channels) {
    ...
    if (remain % 2 == 0) {
      double doubleValue = Math.ceil((double)(packets.get((remain-1)/2) + packets.get(((remain-1)/2) + 1)) / 2);
      System.out.println(doubleValue);
      ans += doubleValue;
      System.out.println(ans + " " + packets.get((remain-1)/2)+ " " + packets.get(((remain-1)/2) + 1));
    } else {
