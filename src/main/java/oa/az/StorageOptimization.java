...
public class StorageOptimization {
  ...
  /**
   * Put all rows of separator into a set and remove separators that belong to h. Do the same for
   * columns of separators. Loop through all the rest of separators and calculate the max difference
   * between each. The result should be the maxRowDiff * maxColDiff.
   *
   * <p>Time complexity: O(n + m) Space complexity: O(n + m)
   *
   * @param n
   * @param m
   * @param h
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        StorageOptimization.storage(3, 2, Arrays.asList(1, 2, 3), Arrays.asList(1, 2)));
  }
}
