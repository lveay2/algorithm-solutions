...
// At Stripe, we send a lot of invoices. For each invoice, we send multiple reminder emails. For
// this question, you will be outputting the subject line of each email we send for customers'
// invoices in sorted order. (Use any library/tool for sorting).

// Your invoicing system will need to be able to configure a reminder schedule. For example, you
// might want to send one email 10 days before the invoice comes out, one email when the invoice
// comes out, one email 10 days before the invoice is due, and one email when the invoice is due,
// which is 30 days from when the invoice first came out. This send schedule could look something
// like this, corresponding to the diagram below:
...
// Given a configuration for sending emails, your input will be an unsorted list of unique customer
// invoices with times for when their `[New]` invoice should come out and how much we are charging
// them in dollars. Your objective is to print out the subject lines of all emails we will send out
// in sorted order by time. Your output should include the time, the type of email, the customer,
// and amount due.
...
class Solution {
  public static void main(String[] args) {
    ...
    System.out.println("\nSolution 1");
    invoicer.sendEmails(customer_invoices);
