...
Optimize your algorithm such that it minimizes the number of calls to
the built-in random function of your language.

Implement the Solution class:
Solution(int n, int[] blacklist) Initializes the object with the
integer n and the blacklisted integers blacklist.
int pick() Returns a random integer in the range [0, n - 1] and
not in blacklist.
...
public class _710_RandomPickWithBlacklist {
  ...
  static class Solution {
    ...
    public int pick() {
      ...
      if (map.containsKey(ranInt)) {
        return map.get(ranInt);
      }
