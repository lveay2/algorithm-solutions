 ...
 *
 * <p>Explanation: Node 1's value is 1, and it has two neighbors: Node 2 and 4. Node 2's value is 2,
 * and it has two neighbors: Node 1 and 3. Node 3's value is 3, and it has two neighbors: Node 2 and
 * 4. Node 4's value is 4, and it has two neighbors: Node 1 and 3.
 *
 * <p>Note:
 *
 * <p>The number of nodes will be between 1 and 100. The undirected graph is a simple graph, which
 * means no repeated edges and no self-loops in the graph. Since the graph is undirected, if node p
 * has node q as neighbor, then node q must have node p as neighbor too. You must return the copy of
